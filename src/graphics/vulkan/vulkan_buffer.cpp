...
void VulkanBuffer::createBuffer(
    ...
    if (psize == 0) {
        throw std::runtime_error(
            "VulkanBuffer::createBuffer() -> buffer size must be non-zero"
        );
    }
    ...
    if (vkCreateBuffer(p_device.getDevice(), &bufferInfo, nullptr, &buffer) !=
        VK_SUCCESS) {
        _device = nullptr;
        throw std::runtime_error(
            "VulkanBuffer::createBuffer() -> failed to create buffer"
        );
    }
    ...
    allocInfo.memoryTypeIndex =
        p_device.findMemoryType(memRequirements.memoryTypeBits, properties);
    ...
    if (vkAllocateMemory(
        ...
        cleanup();
        throw std::runtime_error(
            "VulkanBuffer::createBuffer() -> failed to allocate buffer memory"
        );
    }

    if (vkBindBufferMemory(p_device.getDevice(), buffer, bufferMemory, 0) !=
        VK_SUCCESS) {
        cleanup();
        throw std::runtime_error(
            "VulkanBuffer::createBuffer() -> failed to bind buffer memory"
        );
    }
...
void *VulkanBuffer::map(VkDeviceSize offset, VkDeviceSize mappedSize) {
    ...
    if (_device == nullptr || bufferMemory == VK_NULL_HANDLE) {
        throw std::runtime_error(
            "VulkanBuffer::map() -> buffer has not been created"
        );
    }
    if (_mapped != nullptr) {
        return _mapped;
    ...
    if (vkMapMemory(
        ...
        throw std::runtime_error(
            "VulkanBuffer::map() -> failed to map buffer memory"
        );
    }
...
void VulkanBuffer::unmap() {
    if (_mapped != nullptr && _device != nullptr &&
        _device->getDevice() != VK_NULL_HANDLE) {
