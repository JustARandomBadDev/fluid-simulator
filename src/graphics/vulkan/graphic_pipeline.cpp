...
void GraphicPipeline::createGraphicsPipeline(
    ...
    try {
        ...
        if (vkCreatePipelineLayout(
            ...
            throw std::runtime_error("GraphicPipeline::createGraphicsPipeline() -> failed to create particle pipeline layout");
        }
        ...
        if (vkCreateGraphicsPipelines(
            ...
            throw std::runtime_error(
                "GraphicPipeline::createGraphicsPipeline() -> failed to create particle graphics pipeline from shaders: "
...
void GraphicPipeline::cleanup(Device& p_device) {
    ...
    if (particlePipelineLayout != VK_NULL_HANDLE) {
        vkDestroyPipelineLayout(p_device.getDevice(), particlePipelineLayout, nullptr);
...
VkShaderModule GraphicPipeline::createShaderModule(const std::vector<char>& code, Device& p_device) {
    if (code.empty() || code.size() % sizeof(uint32_t) != 0) {
        throw std::runtime_error("GraphicPipeline::createShaderModule() -> invalid SPIR-V shader code");
    }
...
std::vector<char> GraphicPipeline::readFile(const std::filesystem::path& filename) {
    ...
    if (fileSize <= 0) {
        throw std::runtime_error("GraphicPipeline::readFile() -> shader file is empty: " + filename.string());
    }

    std::vector<char> buffer(static_cast<size_t>(fileSize));
    file.seekg(0);
    if (!file.read(buffer.data(), fileSize)) {
        throw std::runtime_error("GraphicPipeline::readFile() -> failed to read shader file: " + filename.string());
    }
