...
void Instance::createInstance(
    ...
    if (_validation_layers_enabled && (!checkValidationLayerSupport() ||
                                          !checkDebugUtilsExtensionSupport())) {
        std::cerr << "Vulkan validation requested, but the validation layer or "
                     "debug utils extension is unavailable\n";
        _validation_layers_enabled = false;
    }
    ...
    if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS) {
        throw std::runtime_error(
            "Instance::createInstance() -> failed to create Vulkan instance"
        );
    }
}
...
void Instance::createSurface(
    ...
    if (instance == VK_NULL_HANDLE) {
        throw std::runtime_error(
            "Instance::createSurface() -> Vulkan instance is not initialized"
        );
    }

    if (!p_create_surface) {
        throw std::runtime_error(
            "Instance::createSurface() -> surface creation callback is not set"
        );
    }

    if (p_create_surface(instance, surface) != VK_SUCCESS) {
        throw std::runtime_error(
            "Instance::createSurface() -> failed to create Vulkan surface via "
            "host callback"
        );
    }
}
...
void Instance::setupDebugMessenger() {
    ...
    if (CreateDebugUtilsMessengerEXT(
        ...
        throw std::runtime_error(
            "Instance::setupDebugMessenger() -> failed to create Vulkan debug "
            "messenger"
        );
    }
}
...
std::vector<const char *> Instance::getRequiredExtensions(
    ...
    if (p_required_extensions.empty()) {
        throw std::runtime_error(
            "Instance::getRequiredExtensions() -> host did not provide "
