...
void UniformGrid::init(
    ...
    _particle_cell_positions.resize(p_max_particles, glm::ivec3(0));
...
uint32_t UniformGrid::get(glm::uvec3 p_position) const {
    return p_position.x + p_position.y * _dimensions.x +
           p_position.z * _dimensions.x * _dimensions.y;
}

uint32_t
UniformGrid::get(std::size_t p_x, std::size_t p_y, std::size_t p_z) const {
    return get({p_x, p_y, p_z});
}
...
bool UniformGrid::rebuild(const ParticleData &p_data) {
    ...
    for (uint32_t i = 0; i < p_data.count; i++) {
        const glm::vec3 position{p_data.position_x[i],
            p_data.position_y[i],
            p_data.position_z[i]};

        const glm::ivec3 cell_position = positionToCell(position);
        const uint32_t new_cell = get(glm::uvec3(cell_position));

        if (_initialized && i < _particle_count &&
            new_cell != _particle_cells[i])
            changed = true;

        _particle_cells[i] = new_cell;
        _particle_cell_positions[i] = cell_position;
    }

    if (!changed) return false;

    std::fill(_cell_counts.begin(), _cell_counts.end(), 0);
    ...
    std::fill(_current_cell_counts.begin(), _current_cell_counts.end(), 0);

    for (uint32_t i = 0; i < p_data.count; i++) {
        const uint32_t cell = _particle_cells[i];

        const uint32_t destination =
            _cell_offsets[cell] + _current_cell_counts[cell];
