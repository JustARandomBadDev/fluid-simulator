...
class Renderer {
  public:
    ...
    const VkCommandBuffer &getCommandBuffer(uint32_t imageIndex) const {
    ...
    const VkFence &getCurrentInFlightFences() const {
        return inFlightFences[currentFrame];
    }
    const VkFence &getImageInFlightFence(uint32_t imageIndex) const {
    ...
    const VkSemaphore &getCurrentImageAvailableSemaphores() const {
        return imageAvailableSemaphores[currentFrame];
    }
    const VkSemaphore &getRenderFinishedSemaphore(uint32_t imageIndex) const {
